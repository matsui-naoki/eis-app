...
def circuit_fit(frequencies, impedances, circuit, initial_guess, constants={},
    ...
    if not global_opt:
        if 'maxfev' not in kwargs:
            kwargs['maxfev'] = 10000  # 1e4 is sufficient
        if 'ftol' not in kwargs:
            kwargs['ftol'] = 1e-10  # sufficiently strict
        if 'xtol' not in kwargs:
            kwargs['xtol'] = 1e-10  # parameter convergence criterion
...
class BlackBoxOptEIS:
    ...
    def _get_r_range(self, r_name):
        """Get range for a specific R element."""
        return self.r_ranges.get(r_name, self.r_range)
